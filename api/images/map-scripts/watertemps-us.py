...
async def getValidTimestamps(boundaries:ImageBoundaries) -> list:
    """Gets all valid UNIX timestamps for the windSpeed product """
    l.info("Getting timestamps for the radar..")
    times = []

    async with aiohttp.ClientSession() as session:
        url = "https://api.weather.com/v3/TileServer/series/productSet?apiKey=e1f10a1e78da46f5b10a1e78da96f525&filter=waterTemperatureAtlanticPacific"
        async with session.get(url) as r:
            response = await r.json()

            for t in range(0, len(response['seriesInfo']['waterTemperatureAtlanticPacific']['series'])):

                if (t <= 35):
                    time = response['seriesInfo']['waterTemperatureAtlanticPacific']['series'][t]['ts']
...
def downloadRadarTile(url, p, fn):
    ...
    if exists(f"../../../images/maps/watertemps-us/{ts}.png") or exists(f"../../../images/maps/watertemps-us/temp/{ts}.png"):
...
def convertPaletteToWXPro(filepath:str):
    ...
    final_filepath = os.path.join("../../../images/maps/watertemps-us", os.path.basename(filepath))
...
async def makeRadarImages():
    ...
    for i in range(0, len(times)):
        for c in range(0, len(combinedCoordinates)):
            if (not exists(f'../../../images/maps/watertemps-us/{times[i]}.png')) and (not exists (f'../../../images/maps/watertemps-us/temp/{times[i]}.png')):
                urls.append(f"https://api.weather.com/v3/TileServer/tile?product=waterTemperatureAtlanticPacific&ts={str(times[i])}&xyz={combinedCoordinates[c].x}:{combinedCoordinates[c].y}:6&apiKey=e1f10a1e78da46f5b10a1e78da96f525")
                paths.append(f"../../../images/maps/watertemps-us/tiles/{times[i]}")
    ...
    for i in range(0, len(imgsToGenerate)):
        if (not exists(f"../../../images/maps/watertemps-us/{times[i]}.png")) and (not exists(f"../../../images/maps/watertemps-us/temp/{times[i]}.png")):
            ...
            for c in combinedCoordinates:
                path = f"../../../images/maps/watertemps-us/tiles/{times[i]}/{times[i]}_{c.x}_{c.y}.png"
            ...
            imgsToGenerate[i].save(f"../../../images/maps/watertemps-us/temp/{times[i]}.png")
            framesToComposite.append(f"../../../images/maps/watertemps-us/temp/{times[i]}.png") # Store the path so we can composite it using WAND and PIL

            # Remove the tileset as we don't need it anymore!
            rmtree(f'../../../images/maps/watertemps-us/tiles/{times[i]}')
