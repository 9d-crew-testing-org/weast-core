...
async def getValidTimestamps(boundaries: ImageBoundaries) -> list[tuple[int, int]]:
    ...
    async with aiohttp.ClientSession() as session:
        url = "https://api.weather.com/v3/TileServer/series/productSet?apiKey=e1f10a1e78da46f5b10a1e78da96f525&filter=snow24hrFcst"
        async with session.get(url) as r:
            response = await r.json()
            series_data = response['seriesInfo']['snow24hrFcst']['series']
...
def downloadRadarTile(url, p, fn):
    ...
    print(ts)
    if exists(f"../../../images/maps/snowfallQpfForecast-us/{ts}.tiff") or exists(f"../../../images/maps/snowfallQpfForecast-us/temp/{ts}.tiff"):
...
def convertPaletteToWXPro(filepath:str):
    ...
    # 1 - 3 Inches as defined by IStar Wxscan pm/Local/SnowfallQpfForecast.prod
    img.opaque_paint(Color('rgb(8, 193, 230)'), Color('rgb(170,170,170)'), 7000.0)
    img.opaque_paint(Color('rgb(8, 155, 186)'), Color('rgb(170,170,170)'), 7000.0)
    img.opaque_paint(Color('rgb(8, 123, 153)'), Color('rgb(170,170,170)'), 7000.0)
    # 3 - 6 Inches as defined by IStar Wxscan pm/Local/SnowfallQpfForecast.prod
    img.opaque_paint(Color('rgb(161, 145, 255)'), Color('rgb(230,230,230)'), 7000.0)
    # 6 - 12 Inches as defined by IStar Wxscan pm/Local/SnowfallQpfForecast.prod
    img.opaque_paint(Color('rgb(150, 96, 255)'), Color('rgb(218,165,255)'), 7000.0)
    img.opaque_paint(Color('rgb(104, 42, 186)'), Color('rgb(218,165,255)'), 7000.0)
    # 12+ Inches as defined by IStar Wxscan pm/Local/SnowfallQpfForecast.prod
    img.opaque_paint(Color('rgb(255, 73, 145)'), Color('rgb(154,60,255)'), 7000.0)
    img.opaque_paint(Color('rgb(235, 165, 226)'), Color('rgb(154,60,255)'), 7000.0)

    
    img.save(filename=filepath)
    final_filepath = os.path.join("../../../images/maps/snowfallQpfForecast-us", os.path.basename(filepath))
    img.compression = 'lzw'
    img.save(filename=filepath)
...
async def makeRadarImages():
    ...
    for ts, effective_time in times:
        for c in combinedCoordinates:
            output_file = f"{effective_time}.tiff"

            if not exists(f'../../../images/maps/snowfallQpfForecast-us/{output_file}') and not exists(f'../../../images/maps/snowfallQpfForecast-us/temp/{output_file}') and not exists(f'../../../images/maps/snowfallQpfForecast-us/{ts}.tiff'):
                url = f"https://api.weather.com/v3/TileServer/tile/snow24hrFcst?ts={ts}&fts={effective_time}&xyz={c.x}:{c.y}:6&apiKey=e1f10a1e78da46f5b10a1e78da96f525"
                path = f"../../../images/maps/snowfallQpfForecast-us/tiles/{ts}"
    ...
    for i in range(0, len(imgsToGenerate)):
        if (not exists(f"../../../images/maps/snowfallQpfForecast-us/{times[i]}.tiff")) and (not exists(f"../../../images/maps/snowfallQpfForecast-us/temp/{times[i]}.tiff")):
            l.debug(f"Generate frame for {times[i]}")
            for c in combinedCoordinates:
                print(times[i])
                path = f"../../../images/maps/snowfallQpfForecast-us/tiles/{times[i][0]}/{times[i][0]}_{c.x}_{c.y}.png"
            ...
            imgsToGenerate[i].save(f"../../../images/maps/snowfallQpfForecast-us/temp/{times[i][0]}.tiff", compression = 'tiff_lzw')
            framesToComposite.append(f"../../../images/maps/snowfallQpfForecast-us/temp/{times[i][0]}.tiff") # Store the path so we can composite it using WAND and PIL

            # Remove the tileset as we don't need it anymore!
            if exists(f'../../../images/maps/snowfallQpfForecast-us/tiles/{times[i][0]}'):
                rmtree(f'../../../images/maps/snowfallQpfForecast-us/tiles/{times[i][0]}')
    ...
    for img in framesToComposite:
        ...
        imgF = PILImage.open(os.path.join("../../../images/maps/snowfallQpfForecast-us/temp", os.path.basename(img)))
        #base_map = PILImage.open(os.path.join("./resources/i1OffclBm.tiff"))
        #imgF = PILImage.alpha_composite(base_map, imgF)
        #imgF = imgF.convert("RGB")
        imgF.save(os.path.join("../../../images/maps/snowfallQpfForecast-us", os.path.basename(img)), compression='tiff_lzw')
        if os.path.exists(os.path.join("../../../images/maps/snowfallQpfForecast-us/temp", os.path.basename(img))):
            os.remove(os.path.join("../../../images/maps/snowfallQpfForecast-us/temp", os.path.basename(img)))    
        finished.append(img)
