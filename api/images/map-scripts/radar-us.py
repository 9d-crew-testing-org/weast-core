...
def downloadRadarTile(url, p, fn):
    ...
    if exists(f"../../../images/maps/radar-us/{ts}.tiff") or exists(f"../../../images/maps/radar-us/temp/{ts}.tiff"):
...
def convertPaletteToWXPro(filepath:str):
    ...
    final_filepath = os.path.join("../../../images/maps/radar-us", os.path.basename(filepath))
...
async def makeRadarImages():
    ...
    for i in range(0, len(times)):
        for c in range(0, len(combinedCoordinates)):
            if (not exists(f'../../../images/maps/radar-us/{times[i]}.tiff')) and (not exists (f'../../../images/maps/radar-us/temp/{times[i]}.tiff')):
                ...
                paths.append(f"../../../images/maps/radar-us/tiles/{times[i]}")
    ...
    for i in range(0, len(imgsToGenerate)):
        if (not exists(f"../../../images/maps/radar-us/{times[i]}.tiff")) and (not exists(f"../../../images/maps/radar-us/temp/{times[i]}.tiff")):
            ...
            for c in combinedCoordinates:
                path = f"../../../images/maps/radar-us/tiles/{times[i]}/{times[i]}_{c.x}_{c.y}.png"
            ...
            imgsToGenerate[i].save(f"../../../images/maps/radar-us/temp/{times[i]}.tiff", compression = 'tiff_lzw')
            framesToComposite.append(f"../../../images/maps/radar-us/temp/{times[i]}.tiff") # Store the path so we can composite it using WAND and PIL

            # Remove the tileset as we don't need it anymore!
            rmtree(f'../../../images/maps/radar-us/tiles/{times[i]}')
