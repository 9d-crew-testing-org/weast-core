...
async def getValidTimestamps(boundaries:ImageBoundaries) -> list:
    """Gets all valid UNIX timestamps for the sat product """
    l.info("Getting timestamps for the radar..")
    times = []

    async with aiohttp.ClientSession() as session:
        url = "https://api.weather.com/v3/TileServer/series/productSet?apiKey=e1f10a1e78da46f5b10a1e78da96f525&filter=sat"
        async with session.get(url) as r:
            response = await r.json()

            for t in range(0, len(response['seriesInfo']['sat']['series'])):

                if (t <= 35):
                    time = response['seriesInfo']['sat']['series'][t]['ts']
...
def downloadRadarTile(url, p, fn):
    ...
    if exists(f"../../../images/maps/sat-us/{ts}.png") or exists(f"../../../images/maps/sat-us/temp/{ts}.png"):
...
def convertPaletteToWXPro(filepath:str):
    ...
    final_filepath = os.path.join("../../../images/maps/sat-us", os.path.basename(filepath))
...
async def makeRadarImages():
    ...
    for i in range(0, len(times)):
        for c in range(0, len(combinedCoordinates)):
            if (not exists(f'../../../images/maps/sat-us/{times[i]}.png')) and (not exists (f'../../../images/maps/sat-us/temp/{times[i]}.png')):
                urls.append(f"https://api.weather.com/v3/TileServer/tile?product=sat&ts={str(times[i])}&xyz={combinedCoordinates[c].x}:{combinedCoordinates[c].y}:6&apiKey=e1f10a1e78da46f5b10a1e78da96f525")
                paths.append(f"../../../images/maps/sat-us/tiles/{times[i]}")
    ...
    for i in range(0, len(imgsToGenerate)):
        if (not exists(f"../../../images/maps/sat-us/{times[i]}.png")) and (not exists(f"../../../images/maps/sat-us/temp/{times[i]}.png")):
            ...
            for c in combinedCoordinates:
                path = f"../../../images/maps/sat-us/tiles/{times[i]}/{times[i]}_{c.x}_{c.y}.png"
            ...
            imgsToGenerate[i].save(f"../../../images/maps/sat-us/temp/{times[i]}.png")
            framesToComposite.append(f"../../../images/maps/sat-us/temp/{times[i]}.png") # Store the path so we can composite it using WAND and PIL

            # Remove the tileset as we don't need it anymore!
            rmtree(f'../../../images/maps/sat-us/tiles/{times[i]}')
