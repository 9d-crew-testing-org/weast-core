...
async function getLocations(includeAlerts = false) {
  ...
  try {
    ...
    for (const type of dataTypes) {
      ...
      if (cached && cached.data) {
        try {
          await lib.sendI2Data(type.name, cached.data);
        } catch (err) {
         //console.log(`[-] Failed to send data for ${type.name}:`, err.message);
        }
      } else {
        console.warn(`[!] No cached data to send for ${type.name}`);
      }
    }
  } catch (err) {
...
async function start() {
  ...
  for (const type of dataTypes) {
    await fetchDataForType(type, cache);
  }

  await sendData(cache);
