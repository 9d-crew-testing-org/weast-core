...
async function getData(initType, location) {
    // Data has to be returned using the OBS Station (HiRAD Tecci) for I2.
    ...
    const data = await handleDataReq(`https://api.weather.com/v1/geocode/${i2LFR.lat}/${i2LFR.long}/observations/current.xml?language=en-US&units=e&apiKey=e1f10a1e78da46f5b10a1e78da96f525`, false)
    const i2OBJ = require("../objects/cc")({ xml: data.split("<currentObservationsResponse>")[1].split("</currentObservationsResponse>")[0], tecci: i2LFR.primTecci })
    return i2OBJ
}
