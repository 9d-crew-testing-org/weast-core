...
async function getData(initType, location) {
    ...
    const data = await handleDataReq(`https://api.weather.com/v2/indices/achePain/daypart/3day?geocode=${i2LFR.lat},${i2LFR.long}&format=xml&language=en-US&apiKey=e1f10a1e78da46f5b10a1e78da96f525`, false)
    const i2OBJ = require("../objects/aches")({ xml: data.split("<achesPainsIndex12hour>")[1].split("</achesPainsIndex12hour>")[0], coopId: i2LFR.coopId, metadata: data.split("<metadata>")[1].split("</metadata>")[0] })
    return i2OBJ
}
